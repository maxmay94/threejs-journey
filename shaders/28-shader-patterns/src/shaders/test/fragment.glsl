...
float random(vec2 st) {
    return fract(sin(dot(st.xy, vec2(12.9898, 78.233))) * 43875.5453123);
}
...
void main()
{
    ...
    // float strength = 1.0 - step(0.01, abs(distance(vUv, vec2(0.5)) - raduis));

    // Patern 46
    // float strength = cnoise(vUv * 50.0);

    // Patern 47
    // float strength = step(0.1, cnoise(vUv * 20.0));
    ...
    float strength = step(0.1, abs(cnoise(vUv * 20.0)));
